...
Die Schnittstelle zwischen Frontend und Emulator wird mithilfe einer \ac{WASM} API umgesetzt. Hierzu wurde eine Rust-Bibliothek namens \textit{wasm-bindgen} genutzt, die es erlaubt Methoden und Strukturen zwischen JavaScript und Rust zu teilen.
...
In der Datei \textit{lib.rs} werden damit zustandslose Helfermethoden definiert, die von der Web-Applikation aufgerufen werden können:
...
\rust{get_linemap} erstellt ein Mapping von Program Counter auf Quellcodezeile, das von der Webapplikation dazu genutzt wird, die aktuell ausgeführte Codezeile rot zu hinterlegen.

\subsubsection*{disassemble}

Diese Funktion ruft den Disassembler auf und wandelt eine Liste von Bytes wieder um in Assembly-Code.
...
Die Implementierung der Web-Applikation erfolgt in Form einer Angular-Anwendung. Dazu wurden mehrere Components und ein Service erstellt, die die einzelnen Funktionen der Anwendung abdecken.
...
In Abbildung \ref{fig:architecture} wird die Architektur der Applikation veranschaulicht. Die blauen Objekte stellen hier Components dar, das grüne Objekt den Service und die orangenen externe Abhängigkeiten.

\subsection{Components}

\subsubsection{App-Component}

Die App-Component stellt den zentralen Zugangspunkt der meisten Angular-Applikationen dar. Von hier aus werden die anderen Components der Anwendung eingebunden und miteinander verknüpft.
...
In dieses Skelett werden dann die anderen Components eingebunden.

\subsubsection{RAM-Display}

Die RAM-Display-Component stellt den Arbeitsspeicher des Emulators in Form einer 16x16 Matrix dar. Genutzt wird dafür eine einfache HTML-Tabelle, die mithilfe von Stylesheets entsprechend angepasst wird. Der Zugriff auf den Arbeitsspeicher erfolgt durch direkten Zugriff auf den Speicherbereich der WebAssembly-Objekte im Browser über einen Pointer.

Neben der Arbeitsspeicher-Matrix kümmert sich diese Component auch um die Anzeige der verschiedenen Register.

\subsubsection{Load-File-Dialog}

Diese Component wird nicht von Anfang an auf der Seite eingebunden, sondern wird erst dann in Form eines modalen Dialogs angezeigt, wenn der Nutzer auf den \textit{Load}-Button in der Aktionsleiste drückt. Der Dialog zeigt dann ein simples HTML-Formular an, das mit einem \mintinline{html}{<input type="file"/>} eine Dateiauswahl ermöglicht.
...
Alle Components greifen über das Dependency-Injection-Framework von Angular auf den sogenannten Emulator-Service zu. Dieser Service verwaltet den Emulator, der mithilfe des in Kapitel \ref{sec:wasmapi} definierten WebAssembly-Interfaces erstellt wird.

Der Service bietet den Components verschiedene Funktionen an, wie zum Beispiel, Quellcode zu assemblen, was vom \textit{Assemble}-Button in der Aktionsleiste genutzt wird. Zur Ausführung von Programmen wird im Service außerdem die Hauptschleife des Emulators implementiert.
...
Statt einer \rust{while}-Schleife kommt hier die Funktion \ts{window.setInterval()} des Browsers zum Einsatz. Diese Funktion führt eine gegebene andere Funktion periodisch immer wieder mit einem bestimmten Intervall auf. Dies ermöglicht der Webanwendung trotz der \enquote{Endlossschleife} responsiv zu bleiben. Da der Intervall nicht beliebig klein sein kann, gibt es außerdem die Möglichkeit, in jedem \enquote{Schleifendurchlauf} mehrere Prozessorschritte durchzuführen. So kann der Emulator trotzdem so schnell laufen wie es nötig ist.
...
Eine weitere Aufgabe der Methode ist die Emulation von sogenannten \textit{CP/M Syscalls}.
...
Ein wesentliches Feature, was diese Programme nutzen, sind die BDOS Syscalls, die auf einem CP/M System das Ausgeben von Text in einem Terminal ermöglichen \cite{cpmSyscalls}.
...
Sobald der Program Counter den Wert 5 hat, das Programm also an die Syscall Adresse gesprungen ist, wird dieser emuliert. Beim Syscall 2 wird das Zeichen, was sich Register E befindet, ausgegeben. Syscall 9 gibt den String, der sich an der Speicheradresse aus Register DE befindet, so lange aus, bis das Zeichen \enquote{\$} auftritt.