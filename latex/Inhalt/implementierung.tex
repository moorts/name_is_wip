...
Generell gilt, dass die meisten hier vorgestellten Schritte als einzelne Methoden realisiert sind. Ihre Definition folgt dem Schema

\rust{fn method(code: &Vec<String>) -> Result<Vec<String>, &'static str>}, was bedeutet, dass sie auf der Referenz eines Vektors aus Strings operieren und einen neuen Vektor aus Strings zurückgeben. Diesem Vektor werden, je nach Auswertung, entsprechende Zeilen angehängt. Der Rückgabewert wird im folgenden auch als \glqq Ergebnisvektor\grqq{} einer Methode bezeichnet.
...
Damit Instruktionen wie If-Verzweigungen korrekt funktionieren und ausgewertet werden können sind im nächsten Schritt Variablen durch ihre konstanten Werte zu ersetzen. Dadurch, dass Referenzen auf Makros bereits ersetzt und deren lokale Variablen gegebenenfalls umbenannt wurden, ist dieser Schritt relativ trivial, weil alle Namen gleich zu behandeln sind.
...
Die genaue Funktionsweise der Methode \rust{replace_names} aus Zeile zwei ist im Quellcode \ref{lst:param-replace} näher beschrieben. Letztlich beläuft sie sich darauf, die String-Vorkommen der Schlüssel der angegebenen Map in einem gegebenen String mit den entsprechenden Werten zu ersetzen.
...
Damit beim Iterieren über den Code Variablenzuweisungen, die innerhalb eines If-Blocks stehen, entsprechend behandelt werden, bedarf es einer weiteren Überprüfung des Zeileninhalts. Dazu werden zwei boolsche Variablen \rust{in_conditional} und \rust{condition} definiert, die den aktuellen Zustand bei der Verarbeitung repräsentieren. Dazu wird, sobald ein \asm{IF}-Statement passiert wird die entsprechende Variable auf \rust{true} gesetzt. Zusätzlich wird mittels Parser der Wert der Bedingung bestimmt und in der zweiten Variable angegeben, ob dieser ungleich \glqq 0\grqq{} ist (sofern das gegeben ist, gilt die Bedingung als wahr). Sollte eine Zeile nun eine Variablenzuweisung beinhalten, sich aber in einem als \rust{false} evaluierten If-Block befinden, wird die Zuweisung einfach übergangen.
