...
\subsection{Besitz und (De)Referenzieren}
...
Die Einschränkung des Movens würde offensichtlich auf Dauer zum Problem werden, gäbe es keine Möglichkeit Variablen innerhalb anderer Gültigkeitsbereiche zu verwenden, ohne sie davor zu klonen oder jedes Mal als zusätzlichen Rückgabewert zurückzuliefern. Dafür bietet Rust die Möglichkeit zum \glqq Borrowing\grqq{} mittels \glqq Referencing\grqq. Das geschieht mittels vorangestelltem Et-Zeichen (\&). So wird anstelle der Variablen (dem eigentlichen Besitzer) lediglich ein Verweis auf diesen übergeben.
